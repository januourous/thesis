...
\begin{enumerate}
\item Initialize ReplayMemory. An object of the class is created with the given capacity.
\item Initialize the policy network with random weights
\item Clone the policy network, and name it as the target network
\item Then for each episode:
	\begin{enumerate}
	\item Reset the environment (set the arm and object to the starting state).
	\item Then for each step:
		\begin{enumerate}
		\item Select an action, via exploration or exploitation.
		\item Perform the action and observe the reward and the next state.
		\item Store the experience into the ReplayMemory.
		\item Sample a random batch from replay memory.
		\item Get the states from the batch.
		\item Pass the states to the policy network to obtain the Q-values.
		\item Pass the next states to the target network, and calculate the target Q-values.
		\item Calculate the loss between Q-values and target Q-values.
		\item Do gradient descent in the policy network to minimize the loss.
		\item If the required number of steps have passed, update the weights in the target network with the weights in the policy network.
		\end{enumerate}
	\end{enumerate}
